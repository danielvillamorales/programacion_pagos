...
def crearObjeto(**kwargs):
    ...
    pago.emision = datetime.strptime(
        kwargs.get("emision", date.today()), "%d/%m/%Y"
    ).date()
    pago.vencimiento = datetime.strptime(
        kwargs.get("vencimiento", date.today()), "%d/%m/%Y"
    ).date()
        ...
        try:
            ...
            for r in sheet.rows:
                ...
                if pago.empresa in EMPRESAS_PERMITIDAS:
                    ...
                    messages.warning(
                        request,
                        f"Error en la linea {contador} Empresa ({pago.empresa}) no permitida las unicas permitidas son: (ka, dyjon, pulman, pendientes)",
                    )
            ...
            messages.error(
                request,
                f"Error al subir programacion de pagos: (estructura del archivo o nombre de la hoja incorrectos) {e}",
            )
    else:
        messages.warning(
            request, "No tiene permisos para subir la programacion de pagos"
        )
    return render(request, "importar.html")
...
def consulta(request):
    ...
    pagos_nomina = Pagos.objects.filter(
        fecha_pago=date.today(), empresa="nomina"
    ).order_by("estado", "vencimiento", "-valor")
    pagos = Pagos.objects.filter(fecha_pago=date.today(), empresa="ka").order_by(
        "estado", "vencimiento", "-valor"
    )
    pagos_dyjon = Pagos.objects.filter(
        fecha_pago=date.today(), empresa="dyjon"
    ).order_by("estado", "vencimiento", "-valor")
    pagos_pulman = Pagos.objects.filter(
        fecha_pago=date.today(), empresa="pulman"
    ).order_by("estado", "vencimiento", "-valor")

    pagos_rechazados = pagos.filter(estado="9")
    total_rechazados = sum(pago.valor for pago in pagos_rechazados)
    pagos_ok = pagos.filter(estado="1")
    total_pagos_ok = sum(pago.valor for pago in pagos_ok)
...
def aprobar_todo(request):
    if request.user.has_perm("programaciones.aprobar_pago"):
        Pagos.objects.filter(
            Q(fecha_pago=date.today()) & Q(estado="0") & ~Q(empresa="pendientes")
        ).update(estado="1")
        messages.success(request, "Pagos aprobados correctamente")
    else:
        messages.warning(request, "No tiene permisos para aprobar pagos")
...
def pendientes(request):
    pagos = Pagos.objects.filter(
        fecha_pago=date.today(), estado="0", empresa="pendientes"
    ).order_by("vencimiento", "-valor")
    total = sum(pago.valor for pago in pagos)
    # Consulta para obtener el monto total de los pagos por mes
    pagos_por_mes = (
        Pagos.objects.filter(fecha_pago=date.today(), estado="0", empresa="pendientes")
    ...
    return render(
        request,
        "pendientes.html",
        {"pagos": pagos, "total": total, "pagos_por_mes": pagos_por_mes},
    )
...
def pendientes_next(request):
    pagos = Pagos.objects.filter(
        ...
        vencimiento=date.today() + timedelta(days=1),
        estado="0",
        empresa="pendientes",
    ).order_by("vencimiento", "-valor")
    total = sum(pago.valor for pago in pagos)
...
def pagos_aprobados(request):
    ...
    pagos = ProgramacionPagosAprobados.objects.filter(
        fecha_pago=fecha, estado="1"
    ).order_by("revision", "empresa", "-valor")
    ...
    for pago in pagos:
        pago.cuentas_concatenadas = (
            pago.cuentas_concatenadas.replace("|", "<br>")
            if pago.cuentas_concatenadas
            else ""
        )
...
def exportar_clientes(request):
    ...
    fecha = datetime.strptime(request.POST.get("ifecha"), "%Y-%m-%d").date()
    rows = (
        ProgramacionPagosAprobados.objects.filter(
            fecha_pago=fecha, estado__in=["1", "9"]
        )
        .values_list(
            "fecha_pago",
            "empresa",
            ...
            "valor",
            "estado",
            "cuentas_concatenadas",
        )
        .order_by("empresa", "estado")
    )
        ...
        for col_num, value in enumerate(row):
            if isinstance(value, date):
                ...
                ws.write(
                    row_num,
                    col_num,
                    value if col_num != 10 else ESTADO.get(value),
                    font_style,
                )
...
def exportar_acuerdo(request):
    fecha = datetime.strptime(request.POST.get("ifecha"), "%Y-%m-%d").date()
    cuotas = (
        Acuerdos.objects.filter(
            año=fecha.year, mes=fecha.month, dia=fecha.day, estado="1"
        )
        .values_list(
            "año", "mes", "dia", "nit", "proovedoor", "cuota", "observaciones", "estado"
        )
        .order_by("año", "mes", "dia")
    )
    cuentas_bancarias = CuentasBancarias.objects.filter(estado="0").values_list(
        "nit", "banco", "tipo_cuenta", "numero_cuenta", "digito_verificacion"
    )
    df = pd.DataFrame(
        cuotas,
        columns=[
    ...
    df_cuentas = pd.DataFrame(
        cuentas_bancarias,
        columns=["nit", "banco", "tipo_cuenta", "numero_cuenta", "digito_verificacion"],
    )
    df = pd.merge(df, df_cuentas, on=["nit", "nit"], how="left")
    # Fusionar columnas 'banco', 'tipo_cuenta' y 'numero_cuenta' en una sola columna
    df["cuenta_bancarias"] = df[["banco", "tipo_cuenta", "numero_cuenta"]].apply(
    ...
    if request.method == "POST":
        if request.user.has_perm("programaciones.add_cuentasbancarias"):
            ...
            cuenta = CuentasBancarias(
                ...
                digito_verificacion=digito_verificaicon,
            )
            cuenta.save()
...
def cuentas(request):
    if request.method == "POST":
        ...
        cuentas = CuentasBancarias.objects.filter(
            Q(nit=buscar) | Q(proveedor__icontains=buscar)
        )
    else:
...
def totales_ano(request):
    ...
    cuotas = (
        Acuerdos.objects.all()
        .values("año", "mes")
        .annotate(
            total=Sum("cuota"),
            pendiente=Sum(
                Case(
                    When(estado="0", then="cuota"),
                    default=Value(0),
...
def totales_mes(request, anio, mes):
    cuotas = (
        Acuerdos.objects.filter(año=anio, mes=mes)
        .values("año", "mes", "dia")
        .annotate(
            total=Sum("cuota"),
            pendiente=Sum(
    ...
    total_pendientes = Acuerdos.objects.filter(año=anio, mes=mes, estado="0").aggregate(
        total=Sum("cuota")
    )
    total_aprobados = Acuerdos.objects.filter(año=anio, mes=mes, estado="1").aggregate(
        total=Sum("cuota")
    )
    return render(
        request,
...
def detalle_acuerdo(request, anio, mes, dia):
    cuotas = Acuerdos.objects.filter(año=anio, mes=mes, dia=dia).order_by(
        "estado", "año", "mes", "dia"
    )
    for cuota in cuotas:
...
def rechazar_acuerdo(request, id):
    if request.user.has_perm("programaciones.aprobar_pago"):
        ...
        cuota.dia = (
            Acuerdos.objects.filter(año=cuota.año, mes=cuota.mes)
            .order_by("-dia")
            .first()
            .dia
        )
        cuota.save()
        messages.success(
            request, "Cuota rechazada correctamente, se paso para el ultimo dia"
        )
...
def pendientes_acuerdo(request):
    ...
    año = date.today().year
    dia = date.today().day
    acuerdos = Acuerdos.objects.filter(año=año, mes=mes, estado="0").order_by(
        "dia", "-cuota"
    )
    if request.method == "POST":
        ultimo_dia_acuerdos = (
            Acuerdos.objects.filter(año=año, mes=mes).order_by("-dia").first()
        )
        acuerdos_aprobados = Acuerdos.objects.filter(
            año=año, mes=mes, dia=dia, estado="1"
        )
        acuerdos_pendientes = Acuerdos.objects.filter(
            año=año, mes=mes, dia=dia, estado="0"
        )
        if len(acuerdos_aprobados) > 0 and len(acuerdos_pendientes) == 0:
            messages.warning(
                request,
                "Ya existen cuotas aprobadas para este dia no se puede modificar",
            )
